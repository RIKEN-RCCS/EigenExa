...
double dlanst(char *norn, int *n, const double *D, const double *E);
float slanst(char *norn, int *n, const float *D, const float *E);
...
double* A,
int lda) {
  cblas_dger(FS_layout, m, n, alpha, X, incX, Y, incY, A, lda);
}
...
float* A,
int lda) {
  cblas_sger(FS_layout, m, n, alpha, X, incX, Y, incY, A, lda);
}
