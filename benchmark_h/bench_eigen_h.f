...
*     
      call mat_set_h( n, a, nm )
*-
      call MPI_BARRIER( MPI_COMM_WORLD, ierr )
      d1 = MPI_WTIME( )
*
      call eigen_h( n, n, a, nm, w, z, nm,
...
*
      if ( i_inod == 0 ) then
         ...
         print*,"Elapsed time for eigen_H = ",d2-d1," [sec]"
      end if
*-
      ! reproducible test
      istat = 0
...
*
      call mat_set_h( n, a_, nm )
      call MPI_BARRIER( MPI_COMM_WORLD, ierr )
      d3 = MPI_WTIME( )
      call eigen_h( n, n, a_, nm, w_, z_, nm,
     &     m_forward=48, m_backward=128 )
      call MPI_BARRIER( MPI_COMM_WORLD, ierr )
      d4 = MPI_WTIME( )
      if ( i_inod == 0 ) then
...
*
      subroutine test_h(n, a, lda, w, z, ldz)
      use MPI
      use eigen_libs_mod
      implicit none
* 
      integer :: n,lda,ldz, ny,lda_,ldz_
      complex(8) :: a(lda,*),z(ldz,*)
      real(8) :: w(*)
*
      integer :: i,j
      ...
      complex(8),allocatable :: a_(:,:),z_(:,:),work(:)
      real(8),allocatable :: w_(:), rwork(:)
      integer,allocatable :: iwork(:)
      complex(8),allocatable :: a__(:,:),z__(:,:)
      real(8),allocatable :: w__(:)
      real(8) :: w_err_max,z_err_max, rdummy
      complex(8) :: z1, z2, zdummy, z_err, signz
...
! A:cyclic -> A:block(NB)
      call pzgemr2d( n,n, a,1,1,desca, a_,1,1,desca_, ictxt )
      a__(:,:) = a_(:,:)
      ...
      allocate( rwork(lrwork), work(lwork), iwork(liwork), stat=ierr )
      if ( ierr /= 0 ) then
         call MPI_Abort( MPI_COMM_WORLD, 1, ierr )
      endif
     ...
     &        work, lwork, rwork, lrwork, iwork, liwork, info)
      ...
      if ( inod == 1 ) then
         print*,"Elapsed time for PZHEEVD = ",d2-d1," [sec]"
      end if
...
! A:cyclic <- Z:block(NB)
      call pzgemr2d( n,n, z_,1,1,descz_, a,1,1,desca, ictxt )
...
! signz : a unitary rotation factor to a computed eigenvector
      ...
      if ( i_2 <= i_3 ) then
        call MPI_Bcast(work,i_3-i_2+1,MPI_DOUBLE_COMPLEX,
     &       0, x_comm, ierr )
      end if
